    ...
    private List<MathematicalFunction> calculateFunctions(double[] h) {
        ...
        for (int i = 0; i < getSize() - 1; i++) {
            ...
            algebraicPolynomial = AlgebraicPolynomial.pow(p1.copy(), 3).times(c1(i, h))
                    .plus(AlgebraicPolynomial.pow(p.copy(), 3).times(c2(i, h)))
                    .plus(p.copy().times(c3(i, h)))
                    .plus(p1.copy().times(c4(i, h)));
            f.add(new MathematicalFunction(
                    algebraicPolynomial,
                    new DomainFunction(true, getPoints().get(i).getX(), getPoints().get(i + 1).getX(), false)
            ));
        }
